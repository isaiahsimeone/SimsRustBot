...
class TeamPollerService(BusSubscriber, Loggable):
    ...
    async def poll_team(self) -> None:
        ...
        await self.publish("team_info", Message(TeamInfo(team_info=team_info)))
        ...
        if team_info.leader_steam_id == 0:
            if self.last_team_info.leader_steam_id != 0:
                ...
                await self.publish("team_left", Message(TeamLeft()))
        ...
        if team_info.leader_steam_id != 0:
            if self.last_team_info.leader_steam_id == 0:
                ...
                await self.publish("team_joined", Message(TeamJoined()))
            elif self.last_team_info.leader_steam_id != team_info.leader_steam_id:
                self.debug("team leader changed")
                await self.publish("team_leader_changed", Message(TeamLeaderChange(new_leader_steam_id=team_info.leader_steam_id)))
        ...
        for change in member_changes:
            ...
            match change:
                case {"added": member}:
                    await self.publish("team_member_join", Message(TeamMemberJoin(member=member)))
                    self.debug(f"MEMBER ADDED: {member.name}")
                case {"removed": member}:
                    await self.publish("team_member_left", Message(TeamMemberLeft(member=member)))
                    ...
                    for change_key, change_detail in specific_changes.items():
                        match change_key:
                            ...
                            case 'is_online_changed':
                                await self.publish("team_member_connectivity", Message(TeamMemberConnectivity(steam_id=steam_id, is_online=change_detail['to'])))
                                self.debug(f"  Online status changed from '{change_detail['from']}' to '{change_detail['to']}'")
                            case 'is_alive_changed':
                                await self.publish("team_member_vital", Message(TeamMemberVital(steam_id=steam_id, is_alive=change_detail['to'])))
